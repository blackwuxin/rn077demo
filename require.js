...
// 模块加载性能跟踪
const moduleLoadingStack = [];
const moduleLoadingTree = new Map();
const rootModules = new Set();
function createModuleNode(moduleId, moduleName) {
  return {
    moduleId: moduleId,
...
function nodeToString(node, indent) {
  ...
  var result = prefix + connector + node.moduleName + " " + node.duration + "ms";
  if (node.children.length === 0) {
    return result;
  }
  var childrenStr = node.children
    .map(function(child) { return nodeToString(child, indent + 1); })
    .join("\n");
  return result + "\n" + childrenStr;
}
// 全局函数：输出模块加载树
global.__printModuleLoadingTree = function() {
  console.log("\n=== Module Loading Performance Tree ===");
  
  if (rootModules.size === 0) {
    console.log("No modules loaded yet.");
    return;
  }
  
  rootModules.forEach(moduleId => {
    const node = moduleLoadingTree.get(moduleId);
...
function metroRequire(moduleId) {
  ...
  if (module?.verboseName) {
    moduleName = module.verboseName;
  }
  
  // 创建或获取模块节点
  let currentNode = moduleLoadingTree.get(moduleIdReallyIsNumber);
  if (!currentNode) {
    currentNode = createModuleNode(moduleIdReallyIsNumber, moduleName);
    moduleLoadingTree.set(moduleIdReallyIsNumber, currentNode);
    
    // 如果有父模块，添加到父模块的子节点
    if (moduleLoadingStack.length > 0) {
      const parentNode = moduleLoadingStack[moduleLoadingStack.length - 1];
      addChild(parentNode, currentNode);
    } else {
      // 这是根模块
      rootModules.add(moduleIdReallyIsNumber);
    }
  }
  
  // 将当前模块推入栈
  moduleLoadingStack.push(currentNode);
  ...
  let result;
  try {
    result = module && module.isInitialized
      ? module.publicModule.exports
      : guardedLoadModule(moduleIdReallyIsNumber, module);
  } finally {
    // 完成当前模块加载
    moduleLoadingStack.pop();
